        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          ...
          <div>
            <div className="mb-4">
              ...
              <Image src="/logoheader.png" alt="[[BUSINESS_NAME]]" width={160} height={55} className="h-12 w-auto" />
            </div>
            <p className="text-sm mb-4">[[FOOTER_TAGLINE]]</p>
            {/* e.g. "Professional auto detailing in [City], [State]. We bring your vehicle back to life." */}
            <div className="space-y-2 text-sm">
              <div className="flex items-center gap-2"><MapPin className="w-4 h-4 text-primary" /> [[CITY]], [[STATE]]</div>
              <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-primary" /> [[HOURS]]</div>
              {/* e.g. "Mon–Sun: 8 AM – 6 PM" */}
              <div className="flex items-center gap-2"><Phone className="w-4 h-4 text-primary" /> <a href="tel:[[PHONE_LINK]]" className="hover:text-primary transition">[[PHONE_DISPLAY]]</a></div>
              <div className="flex items-center gap-2"><Mail className="w-4 h-4 text-primary" /> <a href="mailto:[[EMAIL]]" className="hover:text-primary transition">[[EMAIL]]</a></div>
            </div>
            {/* [[INSTAGRAM]] — Optional. Remove this block if not needed. */}
            <div className="mt-4">
              <a href="https://www.instagram.com/[[INSTAGRAM_HANDLE]]/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-gray-400 hover:text-primary transition">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              {locations.map((loc) => (
                <li key={loc.slug}>
                  <Link href={`/${loc.slug}`} className="hover:text-primary transition">{loc.name}, [[STATE]]</Link>
                </li>
              ))}
            </ul>
          </div>
